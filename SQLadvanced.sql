...
AS BEGIN
	...
	FROM PhongBan 
	WHERE	
		(@MaPhongBan IS NULL OR MaPhongBan = @MaPhongBan)
		AND
		(@TenPhongBan IS NULL OR TenPhongBan LIKE N'%' + TenPhongBan + '%')
END
...
AS BEGIN
	...
	FROM ChucVu
	WHERE 
		(@MaChucVu IS NULL OR MaChucVu = @MaChucVu)
		AND
		(@TenChucVu IS NULL OR TenChucVu LIKE N'%' + @TenChucVu + '%')
END
